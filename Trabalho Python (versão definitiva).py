...
cursor.execute("CREATE TABLE IF NOT EXISTS Alunos (matrícula TEXT NOT NULL PRIMARY KEY, nome TEXT, email TEXT, nascimento DATE)")
...
while consulta !=10:
 ...
 if consulta == 1:
  ...
  nome5 = str(input("Digite a matrícula do aluno: "))
  cursor.execute("SELECT nome, nascimento, CASE WHEN strftime('%m', date('now')) >  strftime('%m', date(nascimento)) THEN strftime('%Y', date('now')) - strftime('%Y', date(nascimento)) WHEN strftime('%m', date('now')) = strftime('%m', date(nascimento)) THEN CASE WHEN strftime('%d', date('now')) >= strftime('%d', date(nascimento)) THEN strftime('%Y', date('now')) - strftime('%Y', date(nascimento)) ELSE strftime('%Y', date('now')) - strftime('%Y', date(nascimento)) - 1 END WHEN strftime('%m', date('now')) < strftime('%m', date(nascimento)) THEN strftime('%Y', date('now')) - strftime('%Y', date(nascimento)) - 1 END FROM alunos WHERE matrícula='"+nome5+"'") 
  print(cursor.fetchall())
  time.sleep(2)
